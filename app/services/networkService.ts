...
import { Connection } from '@dumpsack/shared-utils';
import { useSettingsStore } from '../state/settingsStore';

/**
 * Get the active RPC URL from settings
 */
export function getActiveRpcUrl(): string {
  ...
  if (!activeNetwork) {
    // Fallback to first network or default
...
/**
 * Create a Solana connection using the active RPC
 */
...
export async function probeLatency(url: string, timeoutMs: number = 5000): Promise<number> {
  ...
  try {
    ...
    // Use a cheap RPC call to test latency
...
/**
 * Probe all networks and update latencies
 */
 ...
 * This will update the active network and trigger a reconnection
 */
